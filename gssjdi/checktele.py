...
« Don't try to chair here @ieFBI »
𓆩 Fishing : successful ✅ !'
⎱UserName: ❲ @{username} ❳
⎱Source @SaQiier
    ''')
                    break
    ...
    {str(eee)}''')
                    if "A wait of" in str(eee):
                        ...
                        await sython.send_message(event.chat.id, " اجاك متاح حاج 🐊")
            else:
                pass
            trys += 1
...
⎱Source @SaQiier
    ...
    {str(eee)}''')
            ...
            isclaim.clear()
            isclaim.append("off")
            await sython.send_message(event.chat_id, "تم الانتهاء من التثبيت حاج ✅")
